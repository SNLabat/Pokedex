...
const sprite = (id, shiny) => `${HOME}${shiny ? "/shiny" : ""}/${id}.png`;
...
const Styles = () => (
  <style>{`
    ...
    @keyframes glow { 0%,100% { box-shadow:0 0 5px rgba(255,215,0,.3) } 50% { box-shadow:0 0 20px rgba(255,215,0,.6) } }
    @keyframes modalIn { from { opacity:0; transform:scale(.9) translateY(20px) } to { opacity:1; transform:scale(1) translateY(0) } }
    @keyframes spriteSwap { 0% { opacity:0; transform:scale(.8) rotateY(90deg) } 100% { opacity:1; transform:scale(1) rotateY(0deg) } }
...
const StatBar = ({ name, value, delay = 0, animate }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
      <span style={{ width: 52, fontSize: 11, fontWeight: 700, color: color, textAlign: "right", letterSpacing: .5 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ position: "relative" }}>
              <div style={{
                ...
                <img key={spriteKey} src={sprite(pokemon.id, shiny)} alt={pokemon.name}
...
export default function PokedexTracker() {
  ...
  const fetchGen = useCallback(async (genIndex) => {
    ...
    try {
      ...
      const cached = ids.filter(id => cache.current.pokemon[id]);
  ...
  // Get all types present in current data
  ...
  return (
    <div style={{
      minHeight: "100vh", fontFamily: "'Inter', 'SF Pro Display', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
      background: "linear-gradient(150deg, #0a0a1a 0%, #12081f 40%, #0a1528 100%)",
      color: "#fff",
    }}>
      <Styles/>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,.4)", marginTop: 2 }}>
              {GENERATIONS[selectedGen].name} Region — Gen {GENERATIONS[selectedGen].label}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} style={{
            ...
            background: "rgba(255,255,255,.04)", color: "#fff", fontSize: 12, cursor: "pointer",
            outline: "none",
          }}>
            <option value="all" style={{ background: "#1a1a2e" }}>All Types</option>
            {availableTypes.map(t => (
              <option key={t} value={t} style={{ background: "#1a1a2e", textTransform: "capitalize" }}>
                {cap(t)}
              </option>
            ))}
          </select>
      ...
      <div style={{ padding: "20px 20px 40px" }}>
        {error ? (
          ...
          <div style={{
            display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(164px, 1fr))", gap: 14,
          }}>
            {filtered.map(p => (
              <PokemonCard
